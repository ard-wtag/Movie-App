  ...
  describe 'PATCH #update' do
    context 'when token is valid' do
      ...
      it 'renders edit when passwords do not match' do
        token = user.signed_id(purpose: 'password_reset', expires_in: 15.minutes)
        patch :update, params: { token: token, user: { password: 'newpassword', password_confirmation: 'wrongpassword' } }
    ...
    context 'when token is invalid' do
      it 'redirects to login with alert' do
        patch :update, params: { token: 'invalid', user: { password: 'newpassword', password_confirmation: 'newpassword' } }
